...
//this function is defined multiple times without #[cfg(feature="LOSCFG_KERNAL_MEM_SLAB_EXTENSION")];
...
fn Os_Mem_System_Init(mem_start: usize) -> u32 {
    ...
    unsafe {
        let m_aucSysMem1 = mem_start as *mut u8;
        /*69 LINE raw*/
    }
    let pool_size = OS_SYS_MEM_SIZE;/*mark*/
    let ret = Los_Mem_Init(m_aucSysMem1, pool_size);
    println!(
...
fn Os_Mem_Find_Suitable_Free_Block(pool: *mut std::ffi::c_void, alloc_size: u32) -> Option<*mut LosMemDynNode> {
    let mut list_node_head: *mut LosDlsList = std::ptr::null_mut();
...
fn Os_Mem_Free_Node(node: *mut LosMemDynNode, pool: *mut LosMemPoolInfo) {
    ...
    // Os_Mem_Reduce_Used(
    //     &mut pool.stat,
    //     Os_Mem_Node_Get_Size!((*node).self_node.size_and_flag.get()),
    ...
    if !Os_Mem_Node_Get_Used_Flag!((*((*node).self_node.prenode)).self_node.size_and_flag.get()) {
        ...
        if !Os_Mem_Node_Get_Used_Flag!((*next_node).self_node.size_and_flag.get()) {
            Os_Mem_List_Delete(&(*next_node).self_node.myunion.free_node_info, first_node);
            Os_Mem_Merge_Node(next_node);
        }
        ...
        Os_Mem_List_Add(list_node_head, (*node).self_node.myunion.free_node_info, first_node);
    }
}
