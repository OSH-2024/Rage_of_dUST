...
fn Os_Mem_Modid_Set(node: *mut LosMemDynNode, module_id: u32) {
    (*node).self_node.myunion.extend_field.moduleid.set(module_id);
    //#[cfg(feature = "LOSCFG_MEM_HEAD_BACKUP")]
    {
        Os_Mem_Node_Save(node);
    }
...
fn Os_Mem_Sys_Node_Check(dst_addr: *mut std::ffi::c_void, src_addr: *mut std::ffi::c_void, node_length: u32, pos: u8)->u32{
    ...
    ret = Los_Mem_Node_Size_Check(pool as *mut std::ffi::c_void, dst_addr, &mut total_size, &mut avail_size);
    ...
    if pos == u8::MAX {
        ...
        ret = Los_Mem_Node_Size_Check(pool as *mut std::ffi::c_void, src_addr, &mut total_size, &mut avail_size);
        if (ret == LOS_OK) && (node_length > avail_size) {
            println!("---------------------------------------------\n");
...
fn Los_Mem_M_Alloc(pool: *mut std::ffi::c_void, size: u32, moduleid: u32) -> *mut std::ffi::c_void {
    ...
    if ptr != std::ptr::null_mut() {
        Mem_Lock!(int_save);
        g_module_mem_used_size[moduleid as usize] = g_module_mem_used_size[moduleid as usize] + Os_Mem_Node_Size_Get(ptr);
...
fn Los_Mem_M_Free(pool: *mut std::ffi::c_void, ptr: *mut std::ffi::c_void, moduleid: u32) -> u32{
    ...
    if ret == LOS_OK {
        Mem_Lock!(int_save);
        g_module_mem_used_size[moduleid as usize] = g_module_mem_used_size[moduleid as usize] - size;
        Mem_Unlock!(int_save);
    }
    return ret;
}
...
fn Los_Mem_M_Realloc(pool: *mut std::ffi::c_void, ptr: *mut std::ffi::c_void, size: u32, moduleid: u32) -> *mut std::ffi::c_void {
    ...
    if new_ptr != std::ptr::null_mut() {
        Mem_Lock!(int_save);
        g_module_mem_used_size[moduleid as usize] = g_module_mem_used_size[moduleid as usize]  +  Os_Mem_Node_Size_Get(new_ptr);
        g_module_mem_used_size[old_module_id as usize] = g_module_mem_used_size[old_module_id as usize] - old_node_size;
